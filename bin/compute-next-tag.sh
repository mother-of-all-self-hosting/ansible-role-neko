...
#
# Tags look like `v<neko version>-<release>`, which is what this repository has
# always published (v2.8.12-0 ... v3.1.5-0):
#
# - if defaults/main.yml points at a Neko version that has never been released,
#   the release counter restarts at 0 (`v3.1.6-0`)
# - otherwise the counter is incremented (`v3.1.6-1`), but only if something
...
# image bump: a fix to the role's tasks or templates published nothing at all,
# which is how tags like v3.0.6-4 had to be cut by hand.
...
# Anchored on `neko_version:` so that none of the variables derived from it -
# `neko_container_image_tag`, `neko_container_image_self_build_repo_version`,
# `neko_container_image_self_build_base_image` - can be mistaken for it.
version="$(sed -nE 's|^neko_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Neko version from $defaults_path"
	exit 1
fi

# Neko's own version is carried without a leading `v` (the `v` lives in the tags
# and in `neko_container_image_self_build_repo_version`), but tolerate one so
